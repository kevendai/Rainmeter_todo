...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/Build-ReleasePackages.ps1 -Version 1.1.5
```

发布到 GitHub 的两个 zip 分工如下：

- [`rainmeter-desktop-widgets-full-1.1.5.zip`](releases/v1.1.5/rainmeter-desktop-widgets-full-1.1.5.zip)：完整功能版，包含待办、今日日程、CalDAV 同步、日程转待办、论文推送和论文标题翻译入口。不要把 `translation.secret`、`paper-sync.secret`、`caldav.secret`、`tasks.json` 或缓存放进包里。
- [`rainmeter-desktop-widgets-lite-1.1.5.zip`](releases/v1.1.5/rainmeter-desktop-widgets-lite-1.1.5.zip)：精简版，包含待办和今日日程，但编译时关闭论文推送与论文标题翻译，并隐藏对应界面入口。CalDAV 功能保留。
