...
8. 已用 Rainmeter + C# 后端接入网页同步的论文评分数据，无需 Python 或 PowerShell 运行时；按摘要分、标题分降序取前五篇。
...
等待用户从桌面实际使用磁贴，根据体感调整宽度、透明度、行高或右上角位置。当网页同步服务产生当日已评分 JSON 后，可点击磁贴右上角 `↻` 做首次真实论文导入验证。
