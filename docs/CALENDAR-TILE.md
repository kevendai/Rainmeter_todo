...
- Calendar 支持新建、编辑和删除本地日程与 CalDAV 日程；CalDAV 仍是远端日程、周期规则和手机提醒的同步来源。
...
- 新建或编辑日程时，周期默认关闭；可选择按日、周、月或年重复。每月可单独选择日期，每年可单独选择月份和日期。
- 每周重复可选择一个或多个星期；周期可永不结束，也可在达到指定重复次数后结束。
- 本地周期日程以系列规则保存在本机并按需展开；CalDAV 周期日程使用标准 `RRULE` 写入远端，后续同步时读取服务器展开的各期。

## 当天与排序

- 仅显示与本地当天 `[00:00, 次日 00:00)` 有交集的事件，跨天事件在涉及的每一天显示。
- 每天零点换日；若零点时电脑休眠，唤醒后的第一次分钟检查补刷新。
- 启动、手动点击 `↻`、换日及每 15 分钟执行一次日程同步。
