...
try {
    ...
    $refs = @('/r:System.Web.Extensions.dll','/r:System.Security.dll')
    ...
    $scaleConfigProbe = Join-Path $build 'ScaleConfigProbe.exe'
    ...
    & $csc /nologo /target:exe /main:ScaleConfigProbe /optimize+ @refs "/out:$scaleConfigProbe" @todoSources (Join-Path $tests 'ScaleConfigProbe.cs')
    if ($LASTEXITCODE -ne 0) { throw 'Scale configuration probe compilation failed' }
    ...
    try {
        ...
        & $calendar UiSaveSelfTest
        if ($LASTEXITCODE -ne 0) { throw 'Calendar WinUI save regression failed' }
        & $plugin SelfTest
        ...
        # v2.1 起文件服务器属于 Paper Snapshot Sync，arxiv 自己不再声明 address_target（计划 §十七）；
        # 老的 arxiv.file_server 目标仍由 SSDP 插件声明着，供未升级的 arxiv 1.x 继续用。
        & $addressProviderProbe 'paper_snapshot.file_server' 'io.github.kevendai.ssdp-server-ip' '100' '203.0.113.7';if($LASTEXITCODE-ne 0){throw 'Paper snapshot file-server address binding failed'}
        ...
        & $arxivPlugin PaperServiceSelfTest;if($LASTEXITCODE -ne 0){throw "Paper service tests failed with exit code $LASTEXITCODE"};Write-Host 'Paper service scoring-parse validation, legacy-secret tolerance and provider availability rules passed'
        # 消费者侧（Phase 9）：服务解析、snapshot-first → attention 流程、付费拒绝语义、旧凭据段不再进入业务配置。
        & $arxivPlugin PaperConsumerSelfTest;if($LASTEXITCODE -ne 0){throw "arXiv plugin consumer tests failed with exit code $LASTEXITCODE"};Write-Host 'arXiv consumer service resolution, attention flow, same-day decline and provider-config hygiene passed'
...
Write-Host 'UpdaterHost EXE and minimal legacy PowerShell launcher passed'
    ...
    try {
        ...
        foreach ($scale in @('0.70','0.75','0.80','0.90','1.00','1.10','1.25')) {
            ...
            Write-Host "Tile scale probe passed at $([int]([double]$scale * 100))%"
        }
    } finally {
        if ($null -eq $previousCommandDisable) { Remove-Item Env:RAINMETER_COMMANDS_DISABLED -ErrorAction SilentlyContinue }
        else { $env:RAINMETER_COMMANDS_DISABLED = $previousCommandDisable }
