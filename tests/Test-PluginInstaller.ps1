...
try {
    ...
    & $Installer -Package $package -PluginRoot (Join-Path $root 'plugins') -HostVersion '2.0.0' -ExpectedSha256 $sha|Out-Null;if($LASTEXITCODE -ne 0){throw 'Valid package was rejected'}
    try{& $Installer -Package $package -PluginRoot (Join-Path $root 'plugins-old-host') -HostVersion '1.9.9' -ExpectedSha256 $sha|Out-Null;throw 'Incompatible host version was accepted'}catch{if($_.Exception.Message -eq 'Incompatible host version was accepted'){throw}}
    try{& $Installer -Package $package -PluginRoot (Join-Path $root 'plugins') -HostVersion '2.0.0' -ExpectedSha256 ('0'*64)|Out-Null;throw 'Bad SHA256 was accepted'}catch{if($_.Exception.Message -eq 'Bad SHA256 was accepted'){throw}}
    ...
    try{& $Installer -Package $bad -PluginRoot (Join-Path $root 'plugins') -HostVersion '2.0.0'|Out-Null;throw 'ZIP traversal was accepted'}catch{if($_.Exception.Message -eq 'ZIP traversal was accepted'){throw}}
