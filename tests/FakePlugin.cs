...
internal static class FakePlugin
{
    ...
    private static int Main()
    {
        ...
        if(action=="progress")Console.WriteLine(Json.Serialize(new Dictionary<string,object>{{"type","progress"},{"request_id",id},{"current",1},{"total",2},{"message","中文进度"}}));Console.WriteLine(Json.Serialize(result));if(action=="duplicate_result")Console.WriteLine(Json.Serialize(result));return 0;
    }
}
