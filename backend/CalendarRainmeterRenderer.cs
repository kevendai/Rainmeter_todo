...
internal static partial class CalendarApp
{
    ...
    private static bool Render(Dictionary<string,object> cache, Dictionary<string,object> state)
    {
        ...
        Meter(lines, "CalendarSummary", "Meter=String", "MeterStyle=StyleText", "X=24", "Y=49", "W=340", "H=16", "FontSize=9", "FontColor=#MutedColor#", "Text=" + now.ToString("M月d日 dddd", CultureInfo.GetCultureInfo("zh-CN")) + "  ·  " + events.Count + " 项");
        int eventHeight = events.Count == 0 ? 54 : events.Sum(e => S(e, "location") == "" ? 44 : 56);
        Meter(lines, "EventSurface", "Meter=Shape", "X=16", "Y=" + y, "Shape=Rectangle 0,0,488," + eventHeight + ",14 | Fill Color #CardColor# | Stroke Color #BorderColor# | StrokeWidth 1");
        if (events.Count == 0)
        {
        ...
        Meter(lines, "CalendarStatus", "Meter=String", "MeterStyle=StyleText", "X=24", "Y=" + (y + 13), "W=470", "H=18", "FontSize=9", "FontColor=#MutedColor#", "Text=" + status, "ToolTipText=" + status); y += 42;
    ...
    {
        Meter(output, "Header", "Meter=String", "MeterStyle=StyleText", "X=24", "Y=18", "W=330", "H=32", "FontSize=18", "FontWeight=600", "Text=今日日程");
