...
internal static partial class CalendarApp
{
    ...
    private static void AppendCalendarChrome(List<string> output)
    {
        ...
        Meter(output, "Sync", "Meter=String", "X=482", "Y=40", "W=36", "H=36", "FontFace=Segoe Fluent Icons", "FontSize=12", "FontColor=225,242,255,255", "StringAlign=CenterCenter", "AntiAlias=1", "Text=\xE72C", "ToolTipText=立即同步 CalDAV", "MouseOverAction=[!SetOption Sync FontColor \"255,255,255,255\"][!UpdateMeter Sync][!Redraw]", "MouseLeaveAction=[!SetOption Sync FontColor \"225,242,255,255\"][!UpdateMeter Sync][!Redraw]", "LeftMouseUpAction=[!SetOption Sync FontColor \"#AccentColor#\"][!SetOption CalendarStatus Text \"正在同步日程…\"][!SetOption CalendarStatus FontColor \"#AccentColor#\"][!UpdateMeter Sync][!UpdateMeter CalendarStatus][!Redraw][\"#@#CalendarHost.exe\" \"Sync\"]");
    }
