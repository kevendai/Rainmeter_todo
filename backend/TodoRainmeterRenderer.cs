...
internal static partial class TodoApp
{
    ...
    private static bool Render(Dictionary<string, object> state)
    {
        ...
        y += 24;
        Meter(lines, "DoneHeading", "Meter=String", "MeterStyle=StyleText", "X=24", "Y=" + y, "W=470", "H=22", "FontSize=10", "FontWeight=600", "FontColor=#MutedColor#", "Text=已办  " + done.Count); y += 28; row = 0;
        List<Dictionary<string, object>> shownDone = done.Take(8).ToList();
        if (shownDone.Count > 0) Meter(lines, "DoneSurface", "Meter=Shape", "X=16", "Y=" + y, "Shape=Rectangle 0,0,488," + (shownDone.Count * 42) + ",14 | Fill Color #DoneCardColor# | Stroke Color #BorderColor# | StrokeWidth 1");
