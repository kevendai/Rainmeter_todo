...
internal static partial class TodoApp
{
    ...
    private static void ShowSettings(int initialPage)
    {
        ...
        run.Click+=delegate{try{PluginManifest m=SelectedRunnablePlugin(list);string verb=m.Capabilities.Contains("value_provider")?"Values":m.Capabilities.Contains("todo_source")?"Sync":"";if(verb=="")throw new Exception("此插件由日历按需调用。 ");if(m.Id=="io.github.kevendai.arxiv"&&verb=="Sync")System.Threading.ThreadPool.QueueUserWorkItem(delegate{try{using(Process process=Process.Start(new ProcessStartInfo(PluginHostPath,"Sync "+m.Id){UseShellExecute=false,CreateNoWindow=true})){if(process!=null){process.WaitForExit();if(PluginAttentionResumable(m.Id))Process.Start(new ProcessStartInfo(Application.ExecutablePath,"PluginConfirmAttention "+m.Id){UseShellExecute=false,CreateNoWindow=true});}}}catch(Exception ex){if(!form.IsDisposed)form.BeginInvoke((Action)(()=>LightUi.Error(ex.Message)));}});else StartPluginCommand(verb,m.Id);status.Text="已启动 "+m.Name;}catch(Exception ex){LightUi.Error(ex.Message);}};
        ...
        if(!String.IsNullOrWhiteSpace(m.Homepage)){ToolStripItem home=menu.Items.Add("打开主页");home.Click+=delegate{RuntimeUtil.Run(m.Homepage);};}if(m.Actions.Count>0)menu.Items.Add(new ToolStripSeparator());foreach(Dictionary<string,object> action in m.Actions){ToolStripItem item=menu.Items.Add(JsonUtil.String(action,"name",JsonUtil.String(action,"id","操作")));item.Tag=action;item.Click+=delegate(object sender,EventArgs ignored){Dictionary<string,object> selectedAction=(Dictionary<string,object>)((ToolStripItem)sender).Tag;string actionId=JsonUtil.String(selectedAction,"id","");string warning=JsonUtil.String(selectedAction,"risk","");if(JsonUtil.Bool(selectedAction,"confirm",false)&&!LightUi.Confirm((warning==""?"确定执行此操作？":warning),"插件操作"))return;ProcessStartInfo start=m.Id=="io.github.kevendai.arxiv"&&actionId=="rescore"?new ProcessStartInfo(Application.ExecutablePath,"PluginRescore "+m.Id):new ProcessStartInfo(PluginHostPath,"PluginAction "+m.Id+" "+actionId);start.UseShellExecute=false;start.CreateNoWindow=true;Process.Start(start);};}menu.Show(actions,new Point(0,actions.Height));}catch(Exception ex){LightUi.Error(ex.Message);}};
